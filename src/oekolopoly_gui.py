...
def get_cached_font(name, size):
    """
    Memoizes pygame.font.SysFont to prevent expensive system font lookups on every frame,
    which otherwise causes massive UI latency.
    """
    key = (name, size)
...
import oeko_core.envs.oeko_env as oeko_env
sys.modules['oeko_core.oeko_core'] = oeko_env
...
try:
    register(
        id='oeko_core-v2',
        entry_point='oeko_core.envs.oeko_env:OekoEnv',
    )
except:
    pass
    ...
    def get_final_action(self, avail, mcts_move=None):
        ...
        while ap + int(V[9]) > 28:
            ...
            if int(V[2]) + dist[2] < 29:
                dist[2] += 1; ap -= 1; changed = True; reasons.append("Burn: Education.")
            elif int(V[3]) + dist[3] < 18:
                dist[3] += 1; ap -= 1; changed = True; reasons.append("Burn: Raising QoL.")
            elif int(V[4]) + dist[4] < 15:
                dist[4] += 1; ap -= 1; changed = True; reasons.append("Burn: Population.")
            elif V[5] < 12:
                if int(V[1]) + dist[1] < 18:
                    dist[1] += 1; ap -= 1; changed = True; reasons.append("Burn: Prod.")
                else: break
            elif V[5] > 20:
                if int(V[0]) + dist[0] < 25:
                    dist[0] += 1; ap -= 1; changed = True; reasons.append("Burn: Cleaning Env.")
            else:
                if int(V[1]) + dist[1] > 5:
                    dist[1] -= 1; ap -= 1; changed = True; reasons.append("Burn: Sabotaging Prod.")
                else: break
        ...
        elif V[2] >= 21: final[5] = 1; reasons.append("Pop: Control birth (Education enables).")
...
class Game:
    def __init__(self, camera, args):
        ...
        self.env = gym.make('oeko_core-v2')
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='Parameters for oeko_core GUI')
