    ...
    for _ in range(60):
        ...
        while ap + int(V[9]) > 28:
            ...
            if int(V[2]) + dist[2] < 29:
                dist[2] += 1; ap -= 1; changed = True
            elif int(V[3]) + dist[3] < 18:
                dist[3] += 1; ap -= 1; changed = True
            elif int(V[4]) + dist[4] < 15:
                dist[4] += 1; ap -= 1; changed = True
            elif V[5] < 12:
                if int(V[1]) + dist[1] < 18:
                    dist[1] += 1; ap -= 1; changed = True
                else: break
            elif V[5] > 20:
                if int(V[0]) + dist[0] < 25:
                    dist[0] += 1; ap -= 1; changed = True
            else:
                if int(V[1]) + dist[1] > 5:
                    dist[1] -= 1; ap -= 1; changed = True
                else: break
        ...
        elif V[2] >= 21: final[5] = 1

        env.unwrapped.step(np.array(final))
