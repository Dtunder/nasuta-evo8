...
def _patched_statistics_step(self, action):
    ...
    try:
        ...
        if terminated or truncated:
            if getattr(self, "_stats_key", "episode") in info:
                info.pop(getattr(self, "_stats_key", "episode"), None)  # Prevent AssertionError collision
            episode_time_length = round(time.perf_counter() - getattr(self, "episode_start_time", time.perf_counter()), 6)
            info[getattr(self, "_stats_key", "episode")] = {
                "r": self.episode_returns,
                "l": self.episode_lengths,
                "t": episode_time_length,
            }
            if hasattr(self, "time_queue"):
        ...
        while not d_done and rounds_played < 50:  # safety cap for rollout simulations
            ...
            if _GLOBAL_SOVEREIGN_MODE:
                ...
                if avail > 0:
                    investments = [a for a in valid_actions if a != 0]
                    if investments:
                        move = np.random.choice(investments)
                    else:
                        move = 0
                else:
                    move = 0
